...
- [x] ログイン機能を実装する。
      → email + password による新規登録 & ログイン。登録時に user_profiles へ upsert。セッションは Cookie で管理（リフレッシュトークン60日）
- [x] ログアウトボタン・設定パネル
      → 右上の ⚙ ボタンから設定パネルを開閉。ログアウトとお問い合わせフォームを内包
- [x] お問い合わせフォーム
      → 設定パネル内。送信内容は contact_messages テーブルに保存
- [ ] アニメーション強化（Framer Motion）
      ・溜まったログに合わせて成長(芽吹→双葉→蕾)
      ・音量に合わせて変化する柔らかい光のアニメーション
- [x] スマホ対応（レスポンシブ調整）
...
| 2026-03 | 認証を Magic Link → email + password に変更。user_profiles 登録フロー追加              |
